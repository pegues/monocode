...
//         "class": "className",
    ...
    getEmDimensions: function(elem) {
        ...
        for (var i=0; i <= 63; i++) {
            fillerX += "\u2588"; // Fill it with a single character (this is a unicode "full block": â–ˆ).  Using the \u syntax because minifiers don't seem to like unicode characters to be in the source as-is.
        }
        for (var i=0; i <= 63; i++) {
    ...
    emulateKeyCombo: function(e) {
        ...
        if (!modifiers.alt && !modifiers.ctrl && modifiers.meta) {
            if (goIn.keyTable[key.string]) {
                ...
                // Macs need this to support pasting with âŒ˜-v (âŒ˜-c doesn't need anything special)

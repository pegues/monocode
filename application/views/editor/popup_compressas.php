...
<html>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function(e) {
                ...
                $('body').on('dblclick', '#local tbody > tr.file', function() {
                    $("#file_name").val($(this).find("td:first-child").html());
                    compress();
                })
                ...
                $("#compressas_form").submit(function(e) {
                    compress();
                    return false;
                });
            });
            ...
            function compress() {
                ...
                var ext = parent.sceditor.call('base.file.getExtension("' + fileName + '")');
                if (ext != 'zip' && ext != 'gz' && ext != 'tar.gz') {
                    parent.sceditor.call("base.notify()", {msg: 'Invalid file type (should be zip, gz or tar.gz).', 'type': 'error'});
                    return;
                }

                var duplicatedObj = $("#local tbody > tr[data-url='" + fileName + "']");

                if (duplicatedObj[0]) {
                    if (duplicatedObj.hasClass('directory')) {
                        parent.sceditor.call("base.notify()", {msg: 'The directory with same name already exists. Please choose another name.', 'type': 'error'});
                    } else {
                        setTimeout(confirmReplacing, 100);
                    }
                    return;
                }
            ...
            function confirmReplacing() {
                $.alert.open({type: 'confirm', content: 'Do you want to replace the existing file?', cancel: true, callback: function(e) {
                        if (e == 'yes') {
                            __compress();
                        }
                    }
            ...
            function __compress() {
                ...
                parent.sceditor.call('base.file.compressAs()', {
                    'dir': directory,
                    'name': fileName,
                }, POPUP_ID);
            }
    ...
    <body data-width='600' data-height="310" data-controls="{'Cancel' : 'closeme','Compress' : 'compress'}" style="height: 100%;">
        <div class="infopopup" style="height: 100%;">
            <form id="compressas_form" class="" style="height: 100%;">

...
<html>
        ...
        <script type="text/javascript">
            ...
            $(document).ready(function (e) {
                ...
                $('body').on('dblclick', '#local tbody>tr.directory', function () {
                    var json = $("div.gridParent#local").data("required");
                    var data = eval("(" + json + ")");

                    if (data.dir != '') {
                        if (data.dir.substr(data.dir.length - 1) != "/") {
                            data.dir += "/";
                        }
                    }

                    data.dir += $(this).data('url');
                    //$("div.gridParent#local").data("required",JSON.stringify(data));
                    
                    parent.sceditor.call("base.file.dir2json()", data, POPUP_ID);
                })

                $('body').on('click', '#local tbody > tr.file', function () {
                    var obj = $(this).find("td:first-child");
                    (obj.find("input").length <= 0) && $("#file_name").val(obj.html());
                });
                ...
                $("body").on('blur focusout ', '.rename', function (e) {
                    ...
                    if ($(this).data('new') != 'yes') {
                        ...
                        $(this).remove();
                        parent.sceditor.call('base.file.rename()', {
                            'dir': directory,
                            'from': old_name,
                            'to': new_name,
                        });
                        ...
                        parent.sceditor.call('base.file.createNew()', {
                            'dir': directory,
                            'name': name,
                            'type': type,
                        }, POPUP_ID);
                ...
                $("body").on("change", "#workspace", function () {
                    ...
                    //$("#local").data("required",JSON.stringify(data));
                    parent.sceditor.call("base.file.dir2json()", data, POPUP_ID);
                })
                ...
                $("body").on('mousedown', 'table.fileGrid tbody>tr', function (e) {
                    ...
                    if ($(this).hasClass('file')) {
                        ...
                        $("#cmroot").find("[data-alias]").removeClass('b-m-idisable');
                        $("#cmroot").find("[data-alias='openFile']").addClass('b-m-idisable');
                ...
                $(".ftpconnfileslist li i").click(function () {
                    ...
                    switch ($(this).data('tool-action')) {
                        ...
                        case "refresh":
                            ...
                            parent.sceditor.call("base.file.dir2json()", _obj, POPUP_ID);
                        ...
                        case "delete":
                            ...
                            parent.sceditor.call("base.file.deleteFiles()", eval("({" + createData + "})"), POPUP_ID);
                ...
                var name = parent.sceditor.call('base.tab.getActiveTitle()', {});
                if (name) {
                    $("#file_name").val(name);
                }
                ...
                try {
                    ...
                    $.each(array, function (key, data) {
                        if (key != 'details' && (getFileName(data.fileUrl).charAt(0) != '.' && $.trim(getFileName(data.fileUrl)) != '.' && $.trim(getFileName(data.fileUrl)) != '..')) {
                            ...
                            if (getExtension(data.fileUrl) != '') {
                                var str = "<tr data-file-id='" + data.fileId + "' data-url='" + data.fileUrl + "' draggable='true' class='" + ext + "'><td>" + getFileName(data.fileUrl) + "</td><td>" + data.fileSize + "</td><td>" + data.lastModifiedDate + "</td></tr>";
                    ...
                    $("div#local.gridParent").data('required', str);
            ...
            function open() {
                ...
                if (!parent.sceditor.call('base.file.isValidFileType("' + fileName + '")')) {
                    parent.sceditor.call("base.notify()", {msg: 'Invalid file type.', 'type': 'error'});
                    return;
                }

                var obj = $("#local tbody > tr[data-url='" + fileName + "']");
                if (obj.length > 0) {
                    ...
                    parent.sceditor.call("base.notify()", {msg: 'File does not exist.', 'type': 'error'});
                    return;
                }
            ...
            function __open() {
                ...
                parent.sceditor.call('base.file.open()', {
                    'id': fileId,
    ...
    <body data-width='600' data-height="310" data-controls="{'Cancel' : 'closeme','Open' : 'open'}" style="height: 100%;">
        <div class="infopopup" style="height: 100%;">
            <form id="open_form" id="" class="" style="height: 100%;">
                                    ...
                                    <div class="ftpconnfiles_inside" style="height: 100%;">
                                        <div style="float: left; margin-bottom: 5px; width: 100%; height: auto;">
                                            ...
                                            <input type="button" 
                                                   ...
                                                   class="button" 
                                                   style="width: auto;" 
                                                   value="Open From Local" 
                                                   id="openfromlocal" />
                                        </div>

                                        <!-- Left Column: Start -->
                                        <div class="ftpconnfiles_col left" style="height: auto; width: 100%;">
                                            <div class="ftpconnfilescol_inside" style="position: relative; height: 225px">
                                                ...
                                                <div class="ftpconnfilescol_navtree gridParent" id="local" style="padding: 30px 0 0; height: 100%;">
            ...
            <div class="clear"></div>
            <form action="<?php echo base_url(); ?>file/openFromLocal" method="post" id="localfileform" style="display: none;" enctype="multipart/form-data">
                <input id="localfile" type="file" name="localfile" onchange="$('#localfileform').submit()" />
            </form>
        </div>
    </body>
</html>
...
class StackoverflowOAuth {
    ...
    public $domain = "https://api.stackexchange.com";

    /**
     * Authorization and AccessToken api endpoints are special in that they live on www.stackoverflow.com not api.stackoverflow.com
     */
    public $authorizationUrl = "https://stackexchange.com/oauth";
    public $accessTokenUrl = "https://stackexchange.com/oauth/access_token";
    ...
    /**
     * API key provided by Stackoverflow for your application
     *
     * @var string
     */
    protected $api_key;
    ...
    /**
     * API secret provided by Stackoverflow for your application
     *
     * @var string
     */
     ...
     *
     * @param string $verification_code the code provided by Stackoverflow
    ...
    public function fetchAccessToken($verification_code, $redirect_uri) {
        ...
        $response = $this->_request($this->accessTokenUrl, $params, 'POST');

        $response['expires_at'] = time() + $response['expires'] - 3600; //Give 1 hour of fudge time for renewal
    ...
    protected function _request($url, array $payload = array(), $method = 'GET') {
        ...
        $errno = curl_errno($ch);
        if ($errno !== 0) {
            throw new Exception(sprintf("Error connecting to Stackoverflow: [%s] %s", $errno, curl_error($ch)), $errno);
        }
        ...
        if (isset($payload['key'])) {
            $data = gzinflate(substr($body, 10, -8));
